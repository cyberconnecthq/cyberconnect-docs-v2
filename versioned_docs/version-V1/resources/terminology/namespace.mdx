...
CyberConnect provides `namespace` as an attribute for each [connection](/v1/concepts/connection/), to identify the source of the connection. You can set only one `namespace` value for each connection and then filter it with the specified `namespace`.
