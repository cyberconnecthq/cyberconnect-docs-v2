...
We've built a [recommendation](/V1/concepts/recommend/) index into our protocol for general follow suggestions. The index jumpstarts by aggregating [connections](/V1/concepts/connection/) from open data sources, including Ethereum blockchain, Foundation.app, Rarible, etc. It generates a personalized "recommended addresses to follow" list for every address.
...
You can find the step by step instructions on how to set up GraphQL client in your application and write a simple **Get Recommendation query** in the [Read Recommendations](/V1/get-started/read-recommendations/) section.
...
For `first` and `after` usage, please refer to [Pagination](/V1/resources/terminology/pagination/) section.
...
`pageInfo` is used to do pagination. Also, you can refer to [Pagination](/V1/resources/terminology/pagination/) section for more details.
...
This is an example of a query to fetch `recommendations` data that you can also test out in the [Playground](/V1/cyberconnect-api/playground/):
