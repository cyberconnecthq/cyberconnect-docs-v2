...
const fetchProfile = async (id: string): Promise<Data> => {
  const res = await fetch(`https://cyberprofile-v2.vercel.app/api/profile/${id}`, {
    method: 'GET',
...
Make sure to check out the full list of fields available in [Profile API](/V1/cyberconnect-api/rest-api/profile/).
