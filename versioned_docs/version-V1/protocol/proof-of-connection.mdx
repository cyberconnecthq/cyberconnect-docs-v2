...
CyberConnect protocol commits to providing a secure and user-friendly process for generating [connections](/V1/concepts/connection/) between addresses.
...
In this page, we use an example from [CyberConnect Solana Page](Now deprecated) to describe the whole process in three parts: key generation, proof generation, and proof verification. The following picture is an example of a complete proof.
