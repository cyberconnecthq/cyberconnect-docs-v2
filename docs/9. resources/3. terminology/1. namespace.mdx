...
<ul>
    <li>If we try to write a new connection without specifying the `namespace`,  the connection will be put into `CyberConnect` namespace.</li>
    <li>If we query the follower list of Alice with `CyberChat` as the namespace parameter, we will get one connection (the orange arrow).</li>
    <li>If we query the follower list of Alice without a namespace parameter or set it as an empty string, we will get three connections.</li>
</ul>
