...
#### `tokenInfo` ([`TokenInfo`](/cyberconnect/reference/objects/token-info))




#### `distanceScore` ([`Float`](/cyberconnect/reference/scalars/float))

`distanceScore` is a measure of how close we think this recommendation matches the input. Lower is better.


#### `rank` ([`Int`](/cyberconnect/reference/scalars/int))
